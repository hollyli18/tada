...
$(document).ready(function(){
	...
	initProjects = function(){
		...
		$('.list-projects .project-item').on('click','.link-project', function(e){

			e.preventDefault();
			currentItem = $(this).parent().parent();
			currentScrollTop = currentItem.offset().top;
		    projectItemClone = currentItem.clone();

			var singleUrl = currentItem.find('.link').attr('href');
			history.pushState({page:singleUrl}, null, singleUrl);

			projectItemClone.addClass('clone').css({
				position:"fixed",
				height: currentItem.outerHeight(),
				top: currentScrollTop - scrollTop,
				left: 0,
				width: "100%"
			})

			$('body').addClass('hideNav showProject loadProject');
			projectItemClone.appendTo( "body" ).delay(30).queue(function(){

				$(this).addClass('active').delay(950).queue(function(){
					...
					$.ajax({ type: "GET",
					    ...
					    success : function(data)
					    {
						    ...
						    console.log('loaded Project');
						    var dataProject = $(data).find('.project');
						   	TweenLite.set(window, {scrollTo: {y: 0, autoKill:false}});

							currentProjectClass = dataProject.data('project');
						    $('body').addClass(currentProjectClass);

						    var new_title = dataProject.data( 'wp-title' );
						    console.log(new_title);
							document.title = "Wokine. "+new_title;

						    $('.page-container').append(dataProject).delay(30).queue(function(){
								$('.page-container .page').not('.project').remove();
								projectItemClone.hide();
								$('body').addClass('addedProject').removeClass('loadProject');
								initProject();
								$(this).dequeue();
							});
		...
		var animationProjectCountPrlx = TweenLite.to(projectCount, 1, { yPercent: -50, ease: Linear.easeNone, paused: true });
		var projectCountIndex = $('.project-count ul');
		var projectCurrentIndex = 0;
		var projectNextIndex;
		$('.project-count ul li').eq(0).delay(50).queue(function(){
			$(this).addClass('active').dequeue();
		});

		//ANIMATIONS COVERS
		var animationPreviewCover = [];
		var animationPreviewFade = [];
		$('.list-projects .project-item').each(function(index){
			var cover = $(this).find('.project-cover .image');
			var fade = $(this).find('.project-cover .fade');
			if (!is_iPad && wW > 992) {
				TweenLite.set(cover, {y: '-25%'});
			}
			animationPreviewCover[index] = TweenLite.to(cover, 1, { y: '25%', ease: Linear.easeNone, paused: true });
			animationPreviewFade[index] = TweenLite.to(fade, 1, { opacity:1 , ease: Linear.easeNone, paused: true });
		});
		...
		function onScroll() {
			...
			if ( !$('body').hasClass('page-projects') || $('body').hasClass('showProject') || wW < 992 || is_iPad ) {
				return;
			};

			//ShowCount
		    if (wH * 0.5 <= scrollTop && scrollTop <= $('.list-projects').offset().top + $('.list-projects').outerHeight() - (wH * 0.5) ) {
			    projectCount.addClass('showed');
		    } else {
			    projectCount.removeClass('showed');
		    }

			//SwitchVisible
			$('.list-projects .project-item').each(function(index){

				if (scrollTop > ( $(this).offset().top - wH * 0.5)  && scrollTop < ($(this).offset().top + $(this).outerHeight() * 0.5) ) {
					projectNextIndex = $(this).index();
				}

				var min = $(this).offset().top - wH;
				var minFade = $(this).offset().top + $(this).outerHeight() * 0.10;
				var max= $(this).offset().top + $(this).outerHeight();
				var norm = clamp(normalize(window.pageYOffset, min, max), 0, 1);
				var normFade = clamp(normalize(window.pageYOffset, minFade, max), 0, 1);
				animationPreviewCover[index].progress(norm);
				animationPreviewFade[index].progress(normFade);

			});

			if (projectCurrentIndex != projectNextIndex) {
				projectCurrentIndex = projectNextIndex;
				projectCountIndex.find('li.active').removeClass('active');
				projectCountIndex.find('li').eq(projectNextIndex).addClass('active');
			}
	...
	initProject = function(){
		...
		var projetcIntroTxt = new SplitText(".project-intro .content p", {type:"lines"});
		...
		tlProject.from(".project-intro .year .waves svg > *", 0.8, {drawSVG:"0%", ease:Power3.easeOut}, 0.5)
		tlProject.from(".project-intro .year .msk", 1, {x:"-20%", opacity:0, ease:Power3.easeOut}, "-=0.60");
		tlProject.staggerFrom($(projetcIntroTitleLines).find('span'), 1.2, {y:"100%", ease: Power3.easeInOut},  0.1, "-=1");
		tlProject.staggerFrom($(projetcIntroLines).find('span'), 1.2, {y:"100%", ease: Power3.easeInOut},  0.1, "-=1");
		tlProject.staggerFrom(".project-intro .list-rewards > li", 0.8, {y:60, opacity:0, ease:Power3.easeOut},  0.1, "-=0.8");
		...
		$('.project').removeClass('waiting').delay(250).queue(function(){
			tlProject.play().timeScale(1);
			$(this).dequeue();
		});

		var projectFocus = $(".project-focus");
		var animationProjectFocusVisuel = [];
		var animationProjectFocusContent = [];
		projectFocus.each(function(index){
			var focusCONTENT = $(this).find('.prllx');
			animationProjectFocusContent[index] = TweenLite.to(focusCONTENT, 1, { y: 400, ease: Linear.easeNone, paused: true });
		});

		TweenLite.ticker.addEventListener("tick", onScroll);

		if($('#swiper-shooting').length > 0) {
			var mySwiper = new Swiper ('#swiper-shooting', {
			    // Optional parameters
			     loop: true,
		...
		$('#full_video .video').click(function(){this.paused?this.play():this.pause();});
		...
		function onScroll() {
			...
			if ( wW >= 992 || !is_iPad ){
				...
				projectFocus.each(function(index) {
				    ...
				    var minCONTENT = $(this).offset().top - wH;
					var max= $(this).offset().top + $(this).outerHeight();
					var normCONTENT = clamp(normalize(window.pageYOffset, minCONTENT, max), 0, 1);

					//animationProjectFocusVisuel[index].progress(normIMG);
					animationProjectFocusContent[index].progress(normCONTENT);

			    });

			}

		    if( $('.project-focus').eq(0).length != 0) {

			    if ($('.navbar-brand').position().top + 20 > $('.project-focus').eq(0).position().top - scrollTop && !$('.navbar-brand').hasClass('dark')) {
				    $('.navbar-brand').addClass('dark');
				    $('.close-project').addClass('dark');
			    } else if ($('.navbar-brand').position().top + 20 < $('.project-focus').eq(0).position().top - scrollTop && $('.navbar-brand').hasClass('dark')) {
				    $('.navbar-brand').removeClass('dark');
				     $('.close-project').removeClass('dark');
			    }

			    if ($('.scrolldown').position().top + 20 > $('.project-focus').eq(0).position().top - scrollTop && !$('.scrolldown').hasClass('dark')) {
				    $('.scrolldown').addClass('dark');
			    } else if ($('.scrolldown').position().top + 20 < $('.project-focus').eq(0).position().top - scrollTop && $('.scrolldown').hasClass('dark')) {
	...
	initIdeas = function(){
		...
		tlRefs.staggerFrom(".list-refs .list-item", 1.2, {y:150, opacity:0, ease: Power3.easeOut},  0.1, "-=1");
		...
		$( '#form-recruitment-candidate' ).submit( function( e ){
			...
			$.ajax({
	            ...
	            success: function( data ) {
	            	...
	            	if( data.status ){
	            		$( '#form-recruitment-candidate input[type="text"]' ).each( function(){
	            			$( this ).val('');
	            		});
	            		$( '#form-recruitment-candidate textarea' ).val('');

	            		if( $( 'html' ).attr( 'lang' ) == 'fr-FR' ){
		            		$( 'input[name="input-file-1"]' ).parent().removeClass( 'filled' );
		            		$( 'input[name="input-file-1"]' ).parent().find( '.file-name' ).html( 'Pièce Jointe 1' );

		            		$( 'input[name="input-file-2"]' ).parent().removeClass( 'filled' );
		            		$( 'input[name="input-file-2"]' ).parent().find( '.file-name' ).html( 'Pièce Jointe 2' );
	            		}
	            		else{
	            			$( 'input[name="input-file-1"]' ).parent().removeClass( 'filled' );
		            		$( 'input[name="input-file-1"]' ).parent().find( '.file-name' ).html( 'Attachment 1' );

		            		$( 'input[name="input-file-2"]' ).parent().removeClass( 'filled' );
		            		$( 'input[name="input-file-2"]' ).parent().find( '.file-name' ).html( 'Attachment 2' );
	            		}
	            	}
		...
		$('body').removeClass('loaded loadProject showProject hideNav').addClass('loading').delay(transitionPageDelay).queue(function(){
			...
			$.ajax({ type: "GET",
			    ...
			    success : function(data)
			    {
					...
					currentProjectClass = currentPage.data('project');
					if (currentProjectClass) {
						console.log('projetclass', currentProjectClass);
						$('body').addClass(currentProjectClass);
					}
